...
class GraphSAGEGraphClassifier(nn.Module):
    ...
    - temporal pooling over each node's lookback window
    - two GraphSAGE layers over the shared graph
    ...
    def __init__(
        ...
        self.sage1 = SAGEConv(self.feature_dim, self.gcn_hidden)
        self.sage2 = SAGEConv(self.gcn_hidden, self.rep_dim)
    ...
    def _resolve_target_positions(
        ...
        if target_node_index is None:
            raise ValueError("target_node_index must be provided for GraphSAGEGraphClassifier")
    ...
    def forward(
        ...
        if graph_edge_index is None:
            raise ValueError("edge_index is required for GraphSAGEGraphClassifier")
        ...
        h = self.sage1(node_feat, graph_edge_index)
        h = self.activation(h)
        h = self.drop(h)

        h = self.sage2(h, graph_edge_index)
        h = self.activation(h)
        h = self.drop(h)
