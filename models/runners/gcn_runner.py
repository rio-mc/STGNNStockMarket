...
from architectures import GCNGraphClassifier
...
class GCNRunner(BaseModelRunner):
    model_name = "GCN"
    ...
    def run(self, app, stock: str, price_df, evaluator, stop_event) -> ModelRunResult:
        ...
        app.logger.info("[GCNRunner] train_ds size=%d", len(train_ds))
        app.logger.info("[GCNRunner] val_ds size=%d", len(val_ds))
        app.logger.info("[GCNRunner] aligned_tickers=%s", aligned_tickers)

        if len(train_ds) == 0:
            raise RuntimeError("GCN training dataset is empty.")
        if len(val_ds) == 0:
            raise RuntimeError("GCN validation dataset is empty.")
