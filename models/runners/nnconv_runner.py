...
class NNConvRunner(BaseModelRunner):
    ...
    def run(self, app, stock: str, price_df, evaluator, stop_event) -> ModelRunResult:
        ...
        Utils.log_gpu_memory("Before NNCONV")
        ...
        app.logger.info("NNCONV parameters: %s", f"{params:,}")
        ...
        Utils.log_gpu_memory("After NNCONV")

        app._check_stop(stop_event)

        app.frontendApp.set_status("Evaluating NNConv graph baseline...")
        ...
        app.frontendApp.set_status("Predicting with NNConv graph baseline...")
        ...
        app.logger.info("[NNConvRunner] %s (%.1f%%)", direction, confidence)
